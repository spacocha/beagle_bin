...
Usage: matfiles_list merge_table lin map output_prefix\n" unless (@ARGV);
($matfile, $mergetable, $linanswer, $mapanswer, $output)=(@ARGV);
chomp ($mergetable);
chomp ($matfile);
chomp ($linanswer);
chomp ($mapanswer);
...
while ($line0 =<MERGE>){
	...
	($sequence)=join ("", @seqs);
	if ($transfahash{$fasta}{$sequence}){
	    die "There's a problem with the unique file $fasta: Not unique $uniquename $transfahash{$fasta}{$sequence}\n";
	} elsif ($seqtranshash{$fasta}{$uniquename}){
	    die "There is a problem with the unique names in $fasta: repeated- $uniquename sequence $sequence otherseqeunce $seqtranshash{$fasta}{$uniquename}\n";
	} else {
	    $seqtranshash{$fasta}{$uniquename}=$sequence;
	    $transfahash{$fasta}{$sequence}=$uniquename;
	}
    }
    close (IN);
    $/="\n";
...
foreach $seq (sort keys %hash){
	...
	if ($transfahash{$transfa}{$seq}){
	    print TRANS "\t$transfahash{$transfa}{$seq}";
	    $totalin++;
	} else {
	    print TRANS "\tNA";
	}

...
if ($oldstats){
	...
	chomp ($line);
	next unless ($line);
	next unless ($line=~/^(.+): Done testing as child/);
...
#    next if ($parenthash{$id}); only find alignment problems for parents
    next if ($skiphash{$id});
	...
	next if ($id eq $oid || $abundhash{$id} >= $abundhash{$oid} || $childhash{$oid});

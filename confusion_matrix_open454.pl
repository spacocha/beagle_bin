...
while ($line1=<IN1>){
	...
	die "Don't know that name structure $name \n";
    }
    $uniquehash{$name}=$unique;
    $abundhash{$name}=$abund;
    print "unique $file1: unique ${unique}\tshrt abund $abund ${name}\tuniquehash $uniquehash{$name}\n" if ($verbose);
}
close (IN1);
...
while ($line1=<IN1>){
	...
	($shrtmock)=$mock=~/^(.+)_[0-9]_[0-9]_[0-9]$/;
	$besthash{$unique}{$shrtmock}++;
	print "$file1: $unique\t$shrtmock\n" if ($verbose);
    }
}
close (IN1);

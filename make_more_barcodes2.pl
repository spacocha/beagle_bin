...
die "Usage: ref_tab old_barcodes no_barcodes length_barcode mismatches_allowed max_failed\n" unless (@ARGV);
($ref, $old, $number, $length, $allowmis, $max) = (@ARGV);
chomp ($ref);
chomp ($old);
chomp ($number);
chomp ($length);
...
foreach $resetseq (sort {$oldhash{$a} <=> $oldhash{$b}} keys %oldhash){
    $done2=();
    $done=();
    until ($done2){
	$done=();
	$seq=$resetseq;
	($basem3, $basem2, $basem1)=$seq=~/(.)(.)(.)$/;
	$baseadded=split ("", $seq);
	$done++ if ($baseadded>=$length);
	until ($done){
	...
	if ($donehash{$seq}){
		...
		die "Missing F1: $f1seq F2: $f2seq F1:$f2check F2:$f2check\n" unless ($f1seq && $f2seq && $f1check && $f2check);
...
#double and triple each base of the first and see if it matches the second                                                                                                             
		...
		if ($failed>$max){
		    ...
		    die "Failed $failed $max: $totalseq R1\n";
		} else{
		    $done=();
		    $done2=();
		}
...
#double and triple each base of the first and see if it matches the second                                                                                                             
		...
		if ($failed>$max){
		    #if you have reach the max number of attempts, reset everything
		    die "Failed $failed $max: $totalseq R2\n";
		} else {
		    $done=();
		    $done2=();
		    $done3=();

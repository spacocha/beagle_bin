...
Usage: unique fnumber output_prefix\n" unless (@ARGV);
($UNIQUE, $FNUMBER, $output)=(@ARGV);
chomp ($UNIQUE);
chomp ($FNUMBER);
chomp ($output);
...
print LOG "Looking for the list files\n" if ($verbose);
if ("-e ${UNIQUE}.*.f${FNUMBER}.*.list"){
    print LOG "Starting to look for error files\n" if ($verbose);
    @errfiles = `ls ${UNIQUE}.*.f${FNUMBER}.*.list`;
    chomp (@errfiles);
    $numbererrs = @errfiles;
    chomp ($numbererrs);
    print LOG "Found $numbererrs list files\n" if ($verbose);

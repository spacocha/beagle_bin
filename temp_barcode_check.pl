...
while ($line=<IN>){
	...
	($well, $refname, $refseq9, $refseq7, $refseq8)=split ("\t", $line);
	if ($donehash{$refseq8}){
	    print "Same seq: |$refname| |$donehash{$refseq8}| |8| |$refseq8|\n";
	} else {
	    $donehash{$refseq8}=$refname;
	}   
    }
}
close (IN);
...
foreach $seq (sort keys %donehash){
    $badbar="GAAGAGC";
    $hitsref=();

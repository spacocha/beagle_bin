...
#   Edited on 08/16/12 
#
$programname="find_seq_errors35.pl";
...
Rfile:$Rfile
fastafile=${fastafile}
logfile:$logfile\n" unless ($output && $matfile && $Rfile && $logfile && $fastafile);
...
if (-e ${logfile}){	
    $resub++;
    open (LOG, ">>${logfile}") or die "${programname}: Can't open ${logfile}\n";
} else {
    open (LOG, ">${logfile}") or die "${programname}: Can't open ${logfile}\n";
}
#define your cutoffs
print "Continuting with resubmission:\n" if ($resub);
...
print LOG "Type of distance matrix:";
if ($dsttype){
    print LOG "$dsttype\n";
} else {
...
if (-e ${output}){
    $init=();
    print LOG "Reading in oldstats file and continuing writing: ${output}\n" if ($verbose);
    open (IN, "<${output}" ) or die "${programname}: Can't open ${output}\n";
    while ($line =<IN>){
	chomp ($line);
	next unless ($line);
...
open (IN, "<$matfile" ) or die "${programname}:Can't open $matfile\n";
$first=1;
while ($line =<IN>){
    chomp ($line);
    next unless ($line);
...
if ($distfiles){
    ...
    print LOG "Proceeding without distances, no clustering will be done\n";
}
...
Open output\n";
if (-e ${output}){
    print LOG "Continue with error file: ${output}\n";
    open (OUT, ">>${output}") or die "${programname}:Can't open $output\n";
} else {
    open (OUT, ">${output}") or die "${programname}:Can't open $output\n";
}
...
foreach $oid (sort {$abundhash{$b} <=> $abundhash{$a}} keys %abundhash){
	...
	if ($abundhash{$oid} < ${abundcutoff}){
	    ...
	    print LOG "$oid is a parent\n" if ($verbose);
	    print OUT "$oid: Done testing as child\n";
	    foreach $mid (sort {$abundhash{$b} <=> $abundhash{$a}} keys %abundhash){
		unless ($mid eq $oid){
		    print LOG "Under mergeall assumptions $mid changed to $oid\n" if ($verbose);
		    print OUT "Changefrom,${mid},${oid},Changeto,DIST,MERGEALL,PVAL,MERGEALL,Done\n";
		    print OUT "$mid: Done testing as child\n";
		}
	    }
	    last;
	} else {
	...
	print LOG "This is id $id and oid $oid\n" if ($verbose);
	if ($gotpvalue){
...
#make the list from the output
($value)= err2list ();
($value)= list2mat ();
($value)=fasta2filter_from_mat ();
...
sub dst2tabpair {
	...
	$isolate2=$otheriso;
	$otheriso2=$isolate;
        if ($isolate2 ne $otheriso2 && ${abundcutoff}*${abundhash{$isolate2}} < $abundhash{$otheriso2} && $distance < $distcutoff && ${abundhash{$isolate2}} <= $abundhash{$otheriso2}){
...
Options:
    ...
    -output                   The output prefix which will be used to create output files. Output files include: .err, .log, .Rfile* (required)
    ...
    -libtype                  Evaluate for specific libaraies. Should include lanename,laneheader comma separated (e.g. Lane1)
    ...
    -fasta                    Fasta file of input sequences for represetative set sequences
\n";
}
...
sub err2list {
    ...
    open (IN, "<${output}" ) or die "${programname}:sub err2list Can't open output ${output}\n";
    open (OUT, ">${output}.list") or die "${programname}:sub err2list Can't open output.list ${output}.list\n";
    while ($line =<IN>){
	chomp ($line);
	next unless ($line);
	if ($line=~/^Changefrom,.+,.+,Changeto,/){
		...
		die "${programname}:sub err2list This parent ($parent) and child ($child) were previously done\n$line";
	    }
	    $done{$child}{$parent}++;
	    die "${programname}:sub err2list Both child and a parent $parent\n" if ($childhash{$parent});
	    $hash{$parent}{$child}++;
	    $childhash{$child}++;
	}
...
sub list2mat {
    ...
    open (IN, "<$matfile" ) or die "${programname}:sub list2mat Can't open matfile $matfile\n";
    open (OUT, ">${output}.mat") or die "${programname}:sub list2mat Can't open output.mat ${output}.mat\n";
    $first=1;
    while ($line =<IN>){
	chomp ($line);
	next unless ($line);
    ...
    open (IN, "<$otulist" ) or die "${programname}:sub list2mat Can't open otulist $otulist\n";
    $first=1;
    while ($line =<IN>){
	chomp ($line);
	next unless ($line);
	if ($type eq "phylo"){
	    ...
	    die "${programname}:sub list2mat- type either mothur eco or phylo\n";
	}
	$OTUname=();
	$mostabund=();
	...
	foreach $oldname (@groups){
		...
		if ($mathashgot{$name}{$head}){
		    ...
		    die "${programname}:sub list2mat Missing name $name from mathash\n";
		}
	    }
	}
    }
...
sub fasta2filter_from_mat {
    
    open (IN, "<${output}.mat" ) or die "${programname}:sub fasta2filter_from_mat Can't open output.mat ${output}.mat\n";
    open (OUT, ">${output}.fa") or die "${programname}:sub fasta2filter_from_mat Can't open output.fa ${output}.fa\n";
    while ($line =<IN>){
	chomp ($line);
	next unless ($line);
    ...
    open (IN, "<${fastafile}" ) or die "${programname}:sub fasta2filter_from_mat Can't open fastafile ${fastafile}\n";
    while ($line =<IN>){
	chomp ($line);
	next unless ($line);
	($info, @seqs) = split ("\n", $line);

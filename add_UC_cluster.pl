...
while ($line=<IN>){
    chomp ($line);
    next unless ($line);
    ($info, $lib, $fseq, $fqual, $rseq, $rqual, $falign, $FreClu) = split ("\t", $line);
    ($bar) = $info =~/^.+\#.*(.{7})$/;
    if ($uchash{$FreClu}){
	#found the uc cluster for this sequence
	print "$info\t$lib\t$fseq\t$fqual\t$rseq\t$rqual\t$falign\t$FreClu\t$uchash{$FreClu}\n";
    } else {
	print "$info\t$lib\t$fseq\t$fqual\t$rseq\t$rqual\t$falign\t$FreClu\tremoved\n";
    }

...
#   Edited on 03/13/12 
#
$programname="find_seq_errors23.pl";
...
until ($i >= $j){
    ...
    if ($ARGV[$i] eq "-d"){
       ...
       $correlcutoff=$ARGV[$k];
       $gotcorrelcutoff++;
   } elsif ($ARGV[$i] eq "-KLdiv"){
       $KLcutoff=$ARGV[$k];
       $gotKLcutoff++;
   } elsif ($ARGV[$i] eq "-smfrac"){
       $smallfrac=$ARGV[$k];
       $gotsmallfrac++;
...
print LOG "Use KLdiv: $KLcutoff\n" if ($gotKLcutoff);
if ($gotKLcutoff){
    die "Required: Define the small fraction to be added to the 0's in KL-dvi test (-smfrac)\n" unless ($gotsmallfrac);
    print LOG "Small Fraction: $smallfrac\n" if ($gotsmallfrac);
}
...
foreach $oid (sort {$abundhash{$b} <=> $abundhash{$a}} keys %abundhash){
	...
	if ($mergeall){
		...
		#although the pvalue suggests splitting, only split if the KL and/or correlation is below cutoff
		if ($gotKLcutoff && $gotcorrelcutoff){
		    ($KLvalue)=KLdiv($id, $oid);
		    ($correl)=findcorrel($id, $oid);
		    if ($KLvalue > $KLcutoff && $correl > $correlcutoff){
			print OUT "Changefrom,$oid,$id,Changeto,DIST,$disthash{$oid}{$id},PVAL,$pvalue,KLdiv,$KLvalue,Correl,$correl,Done\n";
			$parenthash{$id}++;
			$childhash{$oid}++;
			last;
		    }
		    ...
		    ($KLvalue)=KLdiv($id, $oid);
		    if ($KLvalue > $KLcutoff){
			print OUT "Changefrom,$oid,$id,Changeto,DIST,$disthash{$oid}{$id},PVAL,$pvalue,KLdiv,$KLvalue,Done\n";
			$parenthash{$id}++;
			$childhash{$oid}++;
			last;
		    }
...
Options:
    ...
    -KLdiv KLDIV_CUTOFF       Use the KL divergence to determine whether to merge values (default=off), especially when ABUND_CUTOFF = 0.
                              Good KL divergence cut-off 0.2 (best to use when abundance thresh = 0)
    -smfrac SMALLFRAC         A small value (e.g. 0.00001) to be added to the 0 values in the matrix when KL-divergence is used. Should be between 0-1.
\n";
}
...
sub KLdiv {
	...
	if ($abundhash{$id}){
	    if ($mathash{$id}{$bar} && $abundhash{$id}){
		$p=$mathash{$id}{$bar}/$abundhash{$id};
	    } else {
		$p=$smallfrac/$abundhash{$id};
	    }
	} else {
	    die "Missing abundhash{$id} $abundhash{$id}\n";
	}
	if ($abundhash{$oid}){
	    if ($mathash{$oid}{$bar}){
		$q=$mathash{$oid}{$bar}/$abundhash{$oid};
	    } else {
		$q=$smallfrac/$abundhash{$oid};
            }
	} else {
            die"Missing abundhash{$oid} $abundhash{$oid}\n";
	}
	if (($q > 0) && ($p > 0)){
	    $h+=($p*(log($p/$q)));
	}

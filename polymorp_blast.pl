...
while ($line=<IN>){
	...
	until ($i >=$j){
		...
		$mismatchhash{$ref}{$lib}{$i}{$sbases[$i]}+=$mathash{$OTU}{$lib} unless ($sbases[$i] eq $rbases[$i]);
		$totalbases{$ref}{$lib}{$i}+=$mathash{$OTU}{$lib};
		$allbases{$sbases[$i]}++;
		$allbases{$rbases[$i]}++;
	    }
	    $i++;
...
foreach $ref (sort keys %mismatchhash){
    print "$ref\n";
    foreach $lib (sort keys %{$mismatchhash{$ref}}){
	#only make it for one lib at a time
	next unless ($lib eq "H3.mix6" || $lib eq "H12.mix9");
	print "$lib\n";
	print "";
	...
	print "\n";
	foreach $pos (sort {$a <=> $b} keys %{$mismatchhash{$ref}{$lib}}){
	    print "$pos";
	    foreach $base (sort keys %allbases){
		if ($totalbases{$ref}{$lib}{$pos}){
		    if ($mismatchhash{$ref}{$lib}{$pos}{$base}){
			$fractot=$mismatchhash{$ref}{$lib}{$pos}{$base}/$totalbases{$ref}{$lib}{$pos};
			print "\t$fractot";
		    } else {
			print "\t0";
		    }

...
sub print_resub_tnum {
    $finished=();
print RESUB "system(\"perl ~/bin/list_through_filter_from_mat3.pl parallel_unique.final.list ${tnum} parallel_unique.fa parallel_unique.trans 0 parallel_unique.${tnum}.process.f0\\n\");
system (\"mothur \\\"\#align.seqs(template=/data/spacocha/tmp/new_silva_short_unique_1_149.fa,candidate=parallel_unique.${tnum}.process.f0.fa)\\n\");
system (\"FastTree -nt -makematrix parallel_unique.${tnum}.process.f0.align > parallel_unique.${tnum}.process.f0.dst\\n\");
system (\"FastTree -nt -makematrix parallel_unique.${tnum}.process.f0.fa > parallel_unique.${tnum}.process.f0.unaligned.dst\\n\");
system (\"perl ~/bin/find_seq_errors34.pl -d parallel_unique.${tnum}.process.dst,parallel_unique.${tnum}.process.f0.unaligned.dst -m parallel_unique.${tnum}.process.f0.mat -out parallel_unique.${tnum}.process.f0 -dist 0.08\\n\");";
    $finished++;
    return($finished);
}
...
sub print_resub_i {
    $finished=();
    print RESUB "system(\"perl ~/bin/list_through_filter_from_mat3.pl parallel_unique.final.list ${i} parallel_unique.fa parallel_unique.trans 0 parallel_unique.${i}.process.f0\\n\");
open (OUT, \">mothur.command\");
print OUT \"align.seqs(template=/data/spacocha/tmp/new_silva_short_unique_1_149.fa, candidate=parallel_unique.${i}.process.f0.fa)\";
close (OUT);
system (\"mothur<mothur.command\");
system (\"FastTree -nt -makematrix parallel_unique.${i}.process.f0.align > parallel_unique.${i}.process.f0.dst\\n\");
system (\"FastTree -nt -makematrix parallel_unique.${i}.process.f0.fa > parallel_unique.${i}.process.f0.unaligned.dst\\n\");
system (\"rm parallel_unique.${i}.process.f0.log\");
system (\"rm parallel_unique.${i}.process.f0.err\");
system (\"perl ~/bin/find_seq_errors34.pl -d parallel_unique.${i}.process.f0.dst,parallel_unique.${i}.process.f0.unaligned.dst -m parallel_unique.${i}.process.f0.mat -out parallel_unique.${i}.process.f0 -dist 0.08\\n\");\n";
    $finished++;
    return($finished);
}

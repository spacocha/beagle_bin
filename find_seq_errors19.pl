...
Usage: dist mat_file fafile R_file output oldstats\n" unless (@ARGV);
	($distfile, $matfile, $fafile, $Rfile, $output, $oldstats) = (@ARGV);
...
if ($skipalignment){
	...
	next if ($parenthash{$id} || $childhash{$id});
	$gapless2=$seqhash{$id};
	$gapless2=~s/-//g;
	die "No sequence $id $gapless2\n" unless ($gapless2);
	...
	if ($alignmenthash{$gapless2}){
	    ...
	    print "Changefrom,${id},${oid},Changeto,$id,$oid,Alignment mistake,Done\n";
	    $parenthash{$oid}++;
	    $childhash{$id}++;
	} else {
	    $got3=();
		...
		next unless ($gotdist{$oid}{$id} || $abundhash{$oid} > $abundhash{$id});
		if ($gapless3=~/$gapless2/ || $gapless2=~/$gapless3/){
		    print OUT "Changefrom,${id},${oid},Changeto,$id,$oid,Alignment mistake:contained,Done\n";
		    print "Changefrom,${id},${oid},Changeto,$id,$oid,Alignment mistake:contained,Done\n";
		    $parenthash{$oid}++;
		    $childhash{$id}++;
		    $got3++;
		    last;

...
foreach $id (sort {$abundhash{$b} <=> $abundhash{$a}} keys %abundhash){
    ...
    if ($mybesthash{$id}){
	#I already found the right one and printed it
    } else {
	foreach $oid (sort {$mismatchidoid{$id}{$a} <=> $mismatchidoid{$id}{$b}} keys %{$mismatchidoid{$id}}){
	    #choose the one with the fewest mismatches
	    $goodfreq{$id}{$oid}=0 unless ($goodfreq{$id}{$oid});
	    $goodqual{$id}{$oid}=0 unless ($goodqual{$id}{$oid});
	    $badfreq{$id}{$oid}=0 unless ($badfreq{$id}{$oid});
	    $badqual{$id}{$oid}=0 unless ($badqual{$id}{$oid});
	    print "$id\t$oid\tMISMATCH: $mismatchidoid{$id}{$oid}\tGOODFREQ: $goodfreq{$id}{$oid}\tGOODQUAL: $goodqual{$id}{$oid}\tBADFREQ: $badfreq{$id}{$oid}\tBADQUAL: $badqual{$id}{$oid}\n";
	    last;
	}
    }
}

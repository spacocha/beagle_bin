...
#   Edited on 04/02/12 
#
$programname="find_seq_errors27.pl";
...
print LOG "Making transhash from dist files: $distfiles\n" if ($verbose);
...
foreach $oid (sort {$abundhash{$b} <=> $abundhash{$a}} keys %abundhash){
    ...
    %wdisthash=();
    foreach $distfile (@distfilearray){
	print LOG "Reading in the distances from $distfile for $oid\n" if ($verbose);
	($result) = dst2tab ($distfile, $oid);
	if ($result eq "Finished"){
	    ...
	    print "WARNING: Didn't read in the distances for $oid in $distfile\n";
	}
    }
    %done=();

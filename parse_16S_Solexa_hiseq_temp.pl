...
Usage: <Solexa Forward> <seqlenf> <Solexa Reverse> <seqlenr> <Library> > redirect\n" unless (@ARGV);
	($file1, $seqlenf, $file2, $seqlenr, $lib) = (@ARGV);
...
open (IN2, "<${file1}") or die "Can't open forward file ${file1}\n";
	while ($line=<IN2>){
	    chomp ($line);
	    next unless ($line);
	    ($header, $sequence, $qual)=$line=~/^(.+):([A-z]{$seqlenf}):(.+)$/;
	    ($newseq)=$sequence=~/^.{13}(.+)$/;
	    ($newqual)=$qual=~/^.{13}(.+)$/;
	    ($sequence)=$newseq;
	    ($qual)=$newqual;
...
while ($line=<IN3>){
    chomp ($line);
    next unless ($line);
    ($header, $sequence, $qual)=$line=~/^(.+):([A-z]{$seqlenr}):(.+)$/;
    ($shrt_head) = $header =~/^(.+)\/3$/;

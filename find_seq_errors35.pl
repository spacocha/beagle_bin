...
if ($usedist){
    ...
    (@distfilearray)=split (",", $distfiles);
    foreach $file (@distfilearray){
	print LOG "Reading in dist file: $file\n" if ($verbose);
	if ($dsttype){
	    if ($dsttype eq "pair"){
		($answer)=dst2tabpair ($file);
	    } else {
		die "$programname error: does not support this dsttype $dsttype\n";
	    }
	} else {
	    ($answer)=dst2array ($file);
	}
...
foreach $oid (sort {$abundhash{$b} <=> $abundhash{$a}} keys %abundhash){
    ...
    if ($usedist){
	#first, make the disthash for this id
	#clear out any old values
	%disthash=();
	%gotdisthash=();
	#make the disthash
	($answer)= makedisthash();
	die "$programname error: Missing disthash\n" unless ($answer eq "Finished");
...
sub finish_loop{
    ...
    print LOG "$id is the next closest possible parent of $oid $disthash{$oid}{$id}\n" if ($verbose);
...
sub dst2array {
	...
	$isoindhash{$file}{$isolate}=$ind;
	$indisohash{$file}{$ind}=$isolate;
	#get only the lower half of the matrix
	#check to see if these values exist already
	$ind++;
    }
    close (IN);
    $result= "Finished";
...
Example usage:
perl $programname -d file.dst -m file.mat -R R_file -out output

Options:
    -d distance_file          A distance file, or list of distance files (i.e. aligned and unaligned) separated by commas (required)
    ...
    -old                      The program will resume where it left off from previous runs. Output and log files need to be present (optional)
...
sub makedisthash {
    foreach $dfile (@distfilearray){
	($ind)=$isoindhash{$dfile}{$oid};
	$linecount=0;
	...
	while ($dline=<IN>){
	    ...
	    if ($ind==$linecount){
		($info)=shift(@pieces);
		die "$programname error: $info doesn't equal $oid\n" unless ($info eq $oid);
		$newi=0;
		$newj=@pieces;
		until ($newi >=$newj){
		    if ($gotdisthash{$oid}{$indisohash{$dfile}{$newi}}){
			if ($pieces[$newi] < $disthash{$oid}{$indisohash{$dfile}{$newi}}){
			    $disthash{$oid}{$indisohash{$dfile}{$newi}}=$pieces[$newi];
			}
		    } else {
			#the values were not recorded yet
			if ($pieces[$newi] < $distcutoff && $abundhash{$oid} <= $abundhash{$indisohash{$dfile}{$newi}}){
			    $disthash{$oid}{$indisohash{$dfile}{$newi}}=$pieces[$newi];
			    $gotdisthash{$oid}{$indisohash{$dfile}{$newi}}=1;
			}
		    }
		    $newi++;
		}

...
Usage: <Solexa File> <lib_barcode_file> <seqlen> <output_prefix> <yes/no_rc_bar> <logfile> <statfile> <SEEDFILE_output>\n" unless (@ARGV);
	($file1, $barfile, $seqlen, $output, $rcyes, $logfile, $statfile, $seed) = (@ARGV);
...
open (IN2, "<$file1") or die "Can't open $file1\n";
	while ($line=<IN2>){
		chomp ($line);
		next unless ($line);
		($header, $sequence, $qual)=$line=~/^(.+):([A-z]{$seqlen}):(.+)$/;
		$totalreads++;

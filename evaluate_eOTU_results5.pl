...
# Finish this so that it will resubmit the error programs
	    ...
	    print RESUB "\$systemout=\`qsub -cwd -t $tnum ${RESUBFILE} $vars\`;
print \"Submitted: qsub -cwd -t $tnum ${RESUBFILE} $vars\\n\";\n";
	    $unfin++;
	}
    }
...
sub printmissingno {
	...
	if ($total){
	    until ($i >=$total){
		print RESUB "\$systemout=\`qsub -cwd -t $i $EOTUFILE $vars\`;
print \"Submitted qsub -cwd -t $i $EOTUFILE $vars\\n\";\n" unless ($gothash{$i});

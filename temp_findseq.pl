...
#   Edited on 05/01/12 
#
$programname="find_seq_errors28.pl";
...
print "R license Info:
$licinfo\n";
print "Use JSdiv: $JScutoff\n" if ($useJSdiv);
...
print "Matfile\n";
print "Reading in matfile file: $matfile\n" if ($verbose);
...
print "Reading in dist files: $distfiles\n" if ($verbose);

(@distarray)=split (",", $distfiles);
foreach $file (@distarray){
    print "Reading in dist file: $file\n" if ($verbose);
...
foreach $oid (sort {$abundhash{$b} <=> $abundhash{$a}} keys %abundhash){
	...
	if ($abundhash{$oid} < ${abundcutoff}){
	    $mergeall=1;
	    $allparent=$oid;
	    print "All will be merged into $oid\n" if ($verbose); 
	} else {
	    $mergeall=();
	    print "Not under mergeall conditions\n" if ($verbose);
	}
	$init=();
    }
    ...
    print "Checking for doneness: $oid\n" if ($verbose);
    next if ($parenthash{$oid} || $donehash{$oid});
    print "$oid was not previously done\n" if ($verbose);
	...
	if ($mergeall){
	    ...
	    if ($childhash{$id}){
		print "FAILED: $id is already a child\n" if ($verbose);
		next;
	    }
	    ...
	    $pvalue=();
	    if ($totalnolib ==1){
		print "$oid and $id are only in one library\n" if ($verbose);
		...
		if (-e "${Rfile}.fisp"){
		    ...
		    close (RES);
		    $gotpvalue++;
		    print "Got pvalue from fishers: $pvalue $id $oid\n" if ($verbose);
		} else {
		    #now try chisq
		    $cpvalue=();
			...
			close (RES);
			print "Recording pvalue for $id $oid $cpvalue\n" if ($verbose); 
			#now get expected
			@retpie=();
			if (-e "${Rfile}.chiexp"){
				...
				if ($pie=~/^(.+)\t(.+)$/){
				    ...
				    print "WARNING: Unrecognizable exp format $id $oid\n" if ($verbose);
				}
			    }
			    die "$hightot $exptot\n@retpie" if ($exptot > 3000);
			    if ($exptot){
				...
				if ($expercent<0.8){
				    ...
				    print "$expercent value ($hightot over $exptot) is less than the expected 0.8: try to simulate pvalue $id $oid\n" if ($verbose);
				    system ("R --slave --vanilla --silent < ${Rfile}.chisim >> ${Rfile}.err");
				    if (-e "${Rfile}.chisimp"){
					print "got chipsimp $id $oid\n" if ($verbose);
					...
					while ($Rline=<RES>){
					    ...
					    ($pvalue)=$Rline=~/^(.+)/;
					    $gotpvalue++;
					    print  "Got pvalue from simulated chisq: $id $oid $pvalue\n" if ($verbose);
					}
					close (RES);
				    }
				} elsif ($gotchip){
				    ...
				    $pvalue=$cpvalue;
				    $gotpvalue++;
				    print "Got the calculated pvalue for chisq: $id $oid $pvalue\n" if ($verbose);
				}
			    } else {
				print "WARNING: Missing the total expected in percent expected calculation for chisq ID: $id OID: $oid\n";
			    }
			} else {
			    print "WARNING: Found ${Rfile}.chip but not ${Rfile}.chiexp $id $oid\n";
			}
		    } else {
			print "Missing results from regular chisq, trying to simulate chisq: $id $oid\n" if ($verbose);
			system ("R --slave --vanilla --silent < ${Rfile}.chisim >> ${Rfile}.err");
			if (-e "${Rfile}.chisimp"){
			    print "got chipsimp $id $oid\n" if ($verbose);
			    open (RES, "<${Rfile}.chisimp") or die "Can't open ${Rfile}.chisimp\n";
				...
				chomp ($Rline);
				next unless ($Rline);
				print "Rline chisimp $Rline $id $oid\n" if ($verbose);
				($pvalue)=$Rline=~/^(.+)/;
				$gotpvalue++;
				print "Got pvalue from simulated chisq: $id $oid $pvalue\n" if ($verbose);
			    }
			    ...
			    print "WARNING: Tried everything and couldn't get a pvalue from $id $oid\n";
		...
		if ($pvalue > $pvaluecutoff){
		    print "pvalue ($pvalue) is larger than the critical pvalue cutoff value ($pvaluecutoff): change $oid to $id\n" if ($verbose); 
		    unless ($mergeall){
		    ...
		    #calculate JSdiv
		    print "Checking the JSdiv for $id $oid\n" if ($verbose);
		    ($JSres) = JS_value ($oid, $id);
		    if ($JSres < $JScutoff){
			print OUT "Changefrom,$oid,$id,Changeto,DIST,$disthash{$oid}{$id},PVAL,$pvalue,JSdiv,$JSres,Done\n";
			print "JSdiv $JSres is below the cutoff $JScutoff; Change $oid to $id\n" if ($verbose);
			$parenthash{$id}++;
			$childhash{$oid}++;
			last;
		    } else {
			print "JSdiv $JSres is not below the cutoff $JScutoff for $id and $oid\n" if ($verbose);
		    }
		    
		}
	    } else {
		print "WARNING: Didn't record a pvalue for $id $oid\n";
		if ($useJSdiv){
		    #calculate JSdiv
		    print "Checking the JSdiv for $id $oid\n" if ($verbose);
		    ($JSres) = JS_value($oid, $id);
			...
			print "JSdiv is below the JSdiv cutoff $JSres $JScutoff for $id $oid; Changing $oid to $id\n" if ($verbose);
			$childhash{$oid}++;
			$parenthash{$id}++;
			last;
		    } else {
			print "JSdiv is not below the JSdiv cutoff $JSres $JScutoff for $id $oid\n" if ($verbose);
		    }
		}
	    }
	}
	...
	$newmonth=$mon+1;
	$lt= "$newmonth-$mday-$newyear $hour:$min:$sec";
	print "$oid: Done testing as child\n$lt\n";
    }
...
Options:
    ...
    -pval PVALUE_CUTOFF       P-value cutoff above which OTUs will be merged. Default=0.0005

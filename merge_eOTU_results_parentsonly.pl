...
foreach $child (sort keys %childhash){
    #is it ever a parent?
    print "$child";
    if ($parenthash{$child}){
	print " was a parent in:";
	foreach $file (sort keys %{$parenthash{$child}}){
	    print "$file;";
	}
	print "\n";
    } else {
	print " was never a parent\n";
    }
    #is it a child of multiple parents?
    print "$child was a child of:";
    foreach $file (@errfiles){
	if ($childhash{$child}{$file}){
	    print "$childhash{$child}{$file};";
	} else {
	    print "NA;";
	}
    }

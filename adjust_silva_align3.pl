...
until ($i > 50000){
    ...
    ($checkseq2) = $workingseq=~/^.{$i}(.{$rcount})/;
    die "Not ckeckseq1\n" unless ($checkseq1);
    die "No checkseq2\n" unless ($checkseq2);

    if ($checkseq1 eq $frontseq){
	#print "These are your front positions: $i $fcount\n";
	$front=$i;
    }
    if ($checkseq2 eq $backseq){
	#print "These are your back positions: $i $rcount\n";
	$back = $i;
	$i = 50000;
    }
    $i++;
...
foreach $info (keys %hash){
    ($frontend) = $hash{$info}=~/^.{$front}(.{$fcount})/;
    ($backend) = $hash{$info}=~/^.{$back}(.{$rcount})/;

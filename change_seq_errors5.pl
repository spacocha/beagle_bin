...
while ($line =<IN>){
	...
	if (-e "/home/spacocha/fisher.output"){
		...
		close (EXPECT);
		#if 80% of values are not above 5, chisq might be wrong
	...
	if ($chisqpvalue>0.005){
	    $pvalue=$chisqpvalue;
	    $test="R_chisq";
	} elsif ($chisqpvalue<=0.005 && $percent > 0.8){
	    #the chisq approximation is ok, so use it
	    $pvalue=$chisqpvalue;
	    $test="R_chisq";
	} elsif ($percent <= 0.8){
	    #the chisq pvalue is low, but it might be wrong
	    #only merge them if it's really an error
	    $badfound=();
	    $badfreqfound=();
	    foreach $data (@restinfo){
		$badfound++ if ($data=~/BAD/);
		if ($data=~/\((.+)\) than OID \((.+)\)/){
		    ($f1, $f2)=$data=~/\((.+)\) than OID \((.+)\)/;
		    die "F1: $f1 F2: $f2\n" unless ($f1 || $f2);
		    $badfreqfound++ unless ($f1 < 0.01 && $f2 > 0.25);
		}
	    }
	}
	if ($badfreqfound){
	    #only merge them if the qualities are also bad
	    if ($badfound){
		$pvalue=0.004;
		$test="Expect_low_badqual";
	    } else {
		#merge them because both the freqs and qualities are good
		$pvalue=0.006;
		$test="Expect_low_goodqual";
	    }
	} else {
	    #the frequencies of the bad base are very low
	    #merge them because it's probably an error
	    $pvalue=0.006;
	    $test="Expect_low_goodfreq";
	}
    } else{

...
Usage: full_list line_no fasta trans filter_number output_prefix\n" unless (@ARGV);
	
	chomp (@ARGV);
	($full_list, $lineno,  $fullfasta, $fulltrans, $filterno, $output) = (@ARGV);
...
while ($line =<IN>){
    ...
    next unless ($line);
    ($name, $OTU)=split ("\t", $line);
    #only keep the translations that are related to the stored OTUs
    if ($otuhash{$OTU}){
	if ($name=~/^.+_[0-9]+ /){
	    ($lib)=$name=~/^(.+?)_[0-9]+ /;
	    $alllib{$lib}++;
